...
/**
 * Check out module mainly help user check about the order. - We are assuming
 * user will pay when they receive the order.
 **/
...
public class CheckOutModule {
	...
	private void showConfrimation() throws IOException {
		...
		for (Product p : this.sd.getCart().getproducts()) {
			System.out.println(String.format("Product ID: %d ------ Product Name: %s ---- Product Price: %.2f",
		...
		System.out.println("\nPlease type \"Y\" to confirm and \"N\" to quit.");
